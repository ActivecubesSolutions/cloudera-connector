...
public class HiveQueryResultSet extends HiveBaseResultSet {
	...
	public boolean next() throws SQLException {
		...
		fetchedDeque = new LinkedBlockingDeque<List<TRow>>(
				HiveJdbcClient.FETCHED_QUEUE_SIZE);
		fetchedResultQueue = new LinkedBlockingDeque<String>(
				HiveJdbcClient.FETCHED_QUEUE_SIZE * 2);
		...
		while (fetchedDeque.size() > 0 || threadproducer.isAlive()) {
			try {
				if (fetchedDeque.size() > 0) {
					
					Consumer consumer = new Consumer(fetchedDeque.take()
							.iterator());
	...
	class Consumer implements Runnable {
		...
		public Consumer(Iterator<TRow> fetchedRowsItr) {
			this.fetchedRowsItr = fetchedRowsItr;
		}
